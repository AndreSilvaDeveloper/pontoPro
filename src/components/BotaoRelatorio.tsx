...
export default function BotaoRelatorio({ pontos, filtro, resumoHoras }: any) {
  const criarDocPDF = () => {
    ...
    const formatarDataLocal = (dataIso: any) => {
      if (!dataIso) return "--/--/----";
      const str =
        dataIso instanceof Date ? dataIso.toISOString().split("T")[0] : dataIso;
  ...
  return (
    <div className="flex gap-2">
      <button
        ...
        className="flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg font-bold text-sm h-[42px]"
      >
        <Eye size={18} /> Ver
      </button>
      <button
        onClick={baixarPDF}
        className="flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg font-bold text-sm h-[42px]"
      >
