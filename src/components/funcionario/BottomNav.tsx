...
import { Clock, History, FileText, PenTool, MessageSquare, MoreHorizontal } from 'lucide-react';
import { getNotifCount } from './NotificacaoSolicitacao';

const navItems = [
  { href: '/funcionario', label: 'Ponto', icon: Clock, badge: false, badgeKey: '' },
  { href: '/funcionario/historico', label: 'Histórico', icon: History, badge: true, badgeKey: '' },
  { href: '/funcionario/comunicados', label: 'Avisos', icon: MessageSquare, badge: false, badgeKey: 'comunicados' },
  { href: '/funcionario/contracheques', label: 'Holerite', icon: FileText, badge: false, badgeKey: '' },
  { href: '/funcionario/ausencias', label: 'Ausências', icon: PenTool, badge: false, badgeKey: '' },
];
...
export default function BottomNav() {
  ...
  useEffect(() => {
    const fetchComunicados = async () => {
      try {
        ...
        if (res.ok) {
          ...
          const naoLidos = Array.isArray(data) ? data.filter((c: any) => !c.lido).length : 0;
    ...
    return () => window.removeEventListener('comunicados-update', fetchComunicados);
  }, []);
  ...
  return (
    ...
    >
      <div className="max-w-md mx-auto flex justify-around items-center h-16">
        {navItems.map(({ href, label, icon: Icon, badge, badgeKey }) => {
          const isActive = pathname === href;
          const showBadge = badge && notifCount > 0;
          const showComunicadosBadge = badgeKey === 'comunicados' && comunicadosNaoLidos > 0;
