...
import { format, differenceInMinutes, isSameDay, getDay, addDays } from 'date-fns';
import { LogOut, Bell, AlertCircle, ShieldAlert, CalendarDays, TrendingUp, TrendingDown, Clock, Calendar, User, FileText, ExternalLink } from 'lucide-react'; 
...
export default function AdminDashboard() {
  ...
  // === CÁLCULO INTELIGENTE (SUPORTE NOTURNO) ===
  const calcularEstatisticas = () => {
    if (!filtroUsuario) return null;

    const agora = new Date();
    // Pega TODOS os pontos do usuário para garantir pares
    ...
    for (let i = 0; i < pontosOrdenados.length; i++) {
        ...
        if (pEntrada.subTipo === 'ENTRADA' || pEntrada.subTipo === 'VOLTA_ALMOCO') {
            const dataEntrada = new Date(pEntrada.dataHora);
            const diaEntradaStr = format(dataEntrada, 'yyyy-MM-dd');
            
            const pSaida = pontosOrdenados[i+1];

            if (pSaida && (pSaida.subTipo === 'SAIDA' || pSaida.subTipo === 'SAIDA_ALMOCO')) {
                // Par Fechado
                const dataSaida = new Date(pSaida.dataHora);
                const diff = differenceInMinutes(dataSaida, dataEntrada);

                // Regra: Hora conta para o dia da ENTRADA
                if (diaEntradaStr >= dataInicio && diaEntradaStr <= dataFim) {
                    minutosTotalPeriodo += diff;
                }
                
                if (isSameDay(dataEntrada, agora)) minutosHoje += diff;

                i++; 
            } else {
                // Par Aberto (Trabalhando)
                if (isSameDay(dataEntrada, agora)) {
                    const diff = differenceInMinutes(agora, dataEntrada);
                    minutosHoje += diff;
    ...
    const getMetaDoDia = (data: Date) => {
        ...
        const calcDiff = (i:string, f:string) => {
            ...
            if (diff < 0) diff += 1440; // Ajuste noturno na meta
            return diff;
        };
        ...
        <div className="bg-slate-900 p-4 rounded-xl border border-slate-800 flex flex-col md:flex-row gap-4 items-end">
          ...
          <div className="w-full md:w-auto">
             <BotaoRelatorio 
                ...
                nomeEmpresa={empresa.nome} // <--- Passando o nome!
             />
          </div>
        </div>
          ...
          <div className="divide-y divide-slate-800">
            {registrosFiltrados.length > 0 ? registrosFiltrados.map((reg) => (
              <div key={reg.id} className={`p-4 flex flex-col md:grid md:grid-cols-5 md:items-center gap-3 transition-colors ${reg.tipo === 'AUSENCIA' ? 'bg-yellow-900/10 hover:bg-yellow-900/20' : 'hover:bg-slate-800/30'}`}>
                ...
                <div className="flex items-center gap-2 md:block">{reg.tipo === 'PONTO' ? (<><Clock size={14} className="md:hidden text-green-500" /><span className="text-sm font-bold text-green-400">{format(new Date(reg.dataHora), 'HH:mm')}</span><span className="text-[10px] text-slate-500 block uppercase">{reg.subTipo?.replace('_', ' ')}</span></>) : (<span className="text-xs font-bold bg-yellow-600 text-white px-2 py-1 rounded uppercase">{reg.subTipo?.replace('_', ' ')}</span>)}</div>
