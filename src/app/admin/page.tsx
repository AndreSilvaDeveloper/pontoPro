...
export default function AdminDashboard() {
  ...
  const calcularEstatisticas = () => {
    ...
    Object.keys(pontosPorDia).forEach(dia => {
      ...
      for (let i = 0; i < batidas.length; i += 2) {
        ...
        if (saida) {
            minutosNoDia += differenceInMinutes(saida, entrada);
    ...
    return {
      ...
      metaHoje: formatarHoras(metaHojeMinutos),
  ...
  if (loading) {
    return <div className="min-h-screen bg-slate-950 flex items-center justify-center text-slate-500">Carregando painel...</div>;
  }
            ...
            {pontosFiltrados.length > 0 ? pontosFiltrados.map((ponto) => (
              <div key={ponto.id} className="p-4 flex flex-col md:grid md:grid-cols-5 md:items-center gap-3 hover:bg-slate-800/30 transition-colors">
                ...
                <div className="flex items-start gap-2 md:block col-span-1">
                  <MapPin size={14} className="md:hidden text-purple-500 mt-0.5" />
