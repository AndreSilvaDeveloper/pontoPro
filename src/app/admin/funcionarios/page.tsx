...
export default function GestaoFuncionarios() {
  ...
  const excluirFuncionario = async (id: string, nome: string) => {
    if (
      confirm(
        `ATENÇÃO: Tem certeza que deseja excluir ${nome}?\nIsso apagará TODO o histórico de pontos dele.\nEssa ação não pode ser desfeita.`
      )
    ) {
      try {
  ...
  const resetarSenha = async (id: string, nomeFunc: string) => {
    ...
    try {
      await axios.post("/api/admin/funcionarios/resetar-senha", {
        usuarioId: id,
      });
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="grid gap-3">
          ...
          {funcionarios.map((func) => (
            <div
              key={func.id}
              className="bg-slate-900 p-4 rounded-xl border border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:border-slate-600 transition-colors shadow-md"
            >
              <div className="flex items-center gap-4">
                {func.fotoPerfilUrl ? (
                  <img
                    src={func.fotoPerfilUrl}
                    alt={func.nome}
              ...
              <div className="flex gap-2 justify-end">
                <button
                  ...
                  className="p-2.5 bg-purple-900/30 text-purple-400 hover:bg-purple-600 hover:text-white rounded-lg transition-all border border-purple-900/50"
                  title="Editar"
                >
                  <Pencil size={18} />
                </button>
                ...
                <button
                  ...
                  className="p-2.5 bg-red-900/30 text-red-500 hover:bg-red-600 hover:text-white rounded-lg transition-all border border-red-900/50"
            ...
            <div className="bg-slate-900 w-full max-w-4xl rounded-2xl border border-slate-700 shadow-2xl my-8 relative flex flex-col max-h-[90vh]">
              <div className="p-6 border-b border-slate-800 flex justify-between items-center bg-slate-900 rounded-t-2xl sticky top-0 z-10">
                <h2 className="text-xl font-bold text-white flex items-center gap-2">
                  {idEdicao ? (
                    <>
                      <Pencil size={20} className="text-purple-400" /> Editar
                      Funcionário
                    </>
                  ) : (
                    <>
                      <UserPlus size={20} className="text-green-400" /> Novo
                      Funcionário
                    </>
                  )}
                </h2>
                ...
                <form onSubmit={salvar} className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        className="w-full bg-slate-950 border border-slate-700 p-3 rounded-lg text-white focus:border-purple-500 outline-none"
                        value={nome}
                  ...
                  <div className="bg-slate-950 p-4 rounded-xl border border-slate-800">
                    <p className="text-sm text-slate-300 mb-4 font-bold flex items-center gap-2">
                      <Clock size={16} className="text-purple-400" /> Jornada de
                      Trabalho
                    </p>
                    <div className="space-y-3">
                      {["seg", "ter", "qua", "qui", "sex", "sab", "dom"].map(
                        (dia) => (
                          <div
                            key={dia}
                            className="flex flex-col lg:flex-row lg:items-center gap-3 bg-slate-900/50 p-3 rounded-lg border border-slate-800/50"
                          >
                            <div className="flex items-center justify-between lg:w-32">
                              <span className="font-bold uppercase text-xs text-slate-500 w-8">
                                {dia}
                              </span>
                              ...
                              <div className="flex flex-wrap gap-3 flex-1">
                                <div className="flex items-center gap-2 bg-slate-800/50 px-2 py-1 rounded border border-slate-700">
                                  ...
                                  <input
                                    ...
                                    onChange={(e) =>
                                      updateJornada(dia, "e1", e.target.value)
                                    }
                                  ...
                                  <input
                                    ...
                                    onChange={(e) =>
                                      updateJornada(dia, "s1", e.target.value)
                                    }
                                    className="bg-transparent text-white text-xs outline-none"
                                  />
                                </div>
                                <div className="flex items-center gap-2 bg-slate-800/50 px-2 py-1 rounded border border-slate-700">
                                  ...
                                  <input
                                    ...
                                    onChange={(e) =>
                                      updateJornada(dia, "e2", e.target.value)
                                    }
                                    className="bg-transparent text-white text-xs outline-none"
                                  />
                                  <span className="text-slate-600">-</span>
                                  <input
                                    ...
                                    onChange={(e) =>
                                      updateJornada(dia, "s2", e.target.value)
                                    }
                                    className="bg-transparent text-white text-xs outline-none"
                                  />
                                </div>
                  ...
                  <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 border-dashed hover:border-slate-600 transition-colors">
                    <label className="block text-sm text-slate-300 mb-2 font-bold cursor-pointer">
                      <div className="flex items-center gap-2">
                        ...
                        {idEdicao
                          ? "Trocar Foto de Rosto"
                          : "Foto de Rosto (Reconhecimento Facial)"}
                      </div>
                    </label>
                    <input
                      type="file"
                      ...
                      className="block w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-purple-900 file:text-purple-300 hover:file:bg-purple-800 cursor-pointer"
                    />
                  </div>
                  ...
                  <div className="bg-slate-950 p-4 rounded-xl border border-slate-800">
                    <div className="flex justify-between items-center mb-3">
                      <p className="text-sm text-slate-300 font-bold flex items-center gap-2">
                        <MapPin size={16} className="text-purple-400" /> Regra
                        de Localização
                      </p>
                      <button
                        type="button"
                        onClick={pegarLocalizacaoAtual}
                        className="flex items-center gap-2 bg-purple-600 hover:bg-purple-700 px-3 py-1.5 rounded-lg text-xs font-bold transition-colors"
                      >
                    ...
                    <div className="flex gap-3">
                      <div className="flex-1">
                        ...
                        <input
                          value={lat}
                          readOnly
                          className="w-full bg-slate-900 border border-slate-700 p-2 rounded text-slate-400 text-xs"
                        />
                      </div>
                      <div className="flex-1">
                        <label className="text-[10px] text-slate-500 block mb-1">
                      ...
                      <div className="w-24">
                        ...
                        <input
                          ...
                          className="w-full bg-slate-900 border border-slate-700 p-2 rounded text-white text-xs focus:border-purple-500 outline-none"
                        />
                      </div>
                    </div>
                  </div>
