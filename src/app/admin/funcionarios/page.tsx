...
export default function GestaoFuncionarios() {
  ...
  return (
    <div className="min-h-screen bg-page text-text-primary relative overflow-hidden" style={{ paddingTop: 'env(safe-area-inset-top, 0px)' }}>
      ...
      <div className="max-w-6xl mx-auto p-4 md:p-8 pb-24 space-y-6 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-3 rounded-xl font-bold text-sm flex gap-2 items-center shadow-lg shadow-blue-900/20 transition-colors active:scale-95 disabled:opacity-50"
            ...
            <button
              ...
              className="bg-emerald-600 hover:bg-emerald-500 text-white px-4 py-3 rounded-xl font-bold text-sm flex gap-2 items-center shadow-lg shadow-emerald-900/20 transition-all active:scale-95"
              title="Importar funcionários via CSV"
            >
              <FileSpreadsheet size={18} />
              <span className="hidden sm:inline">Importar</span>
            </button>
            <button onClick={handleNovo} className="bg-emerald-600 hover:bg-emerald-500 text-white px-4 py-3 rounded-xl font-bold text-sm flex gap-2 items-center shadow-lg shadow-emerald-900/20 transition-all active:scale-95">
        ...
        <div className="grid gap-3 animate-in fade-in slide-in-from-bottom-4 duration-500" style={{ animationDelay: '100ms' }} >
          ...
          {!carregandoLista && funcionarios.length === 0 && !buscaNome.trim() && (
          ...
          {listaFiltrada.map(func => (
            <div key={func.id} className="bg-surface backdrop-blur-sm p-4 rounded-2xl border border-border-subtle flex flex-col gap-4 shadow-md hover:border-border-default hover:shadow-lg transition-all">
              ...
              {confirmandoExclusao === func.id ? (
                <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-3 flex items-center justify-between gap-3 animate-in fade-in duration-200">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => excluirFuncionario(func.id)} className="px-3 py-1.5 bg-red-600 hover:bg-red-500 text-white rounded-lg text-xs font-bold transition-colors">Sim</button>
