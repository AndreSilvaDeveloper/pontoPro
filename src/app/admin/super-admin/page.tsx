...
import { Building2, Key, User, Lock, CheckCircle } from 'lucide-react';
...
export default function SuperAdminPage() {
  ...
  const criarCliente = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Limpa formulário (exceto a chave mestra pra facilitar o próximo)
    ...
    <div className="min-h-screen bg-black text-white p-6 flex items-center justify-center">
      <div className="w-full max-w-2xl space-y-8">
        ...
        <div className="text-center">
          <h1 className="text-3xl font-bold text-purple-500">Painel Super Admin 🚀</h1>
          <p className="text-gray-400">Cadastre novas empresas e entregue o acesso.</p>
        </div>

        {/* ÁREA DE SUCESSO (RESULTADO) */}
        {resultado && (
          <div className="bg-green-900/30 border border-green-500 p-6 rounded-xl animate-pulse">
            <div className="flex items-center gap-2 text-green-400 mb-4">
              <CheckCircle size={24} />
              <h2 className="text-xl font-bold">Cliente Criado!</h2>
            </div>
            <div className="bg-black p-4 rounded text-sm font-mono space-y-2">
              <p><span className="text-gray-500">Empresa:</span> {resultado.dados.empresa}</p>
            ...
            <p className="text-xs text-green-300 mt-4 text-center">Copie e mande no WhatsApp do cliente!</p>
          </div>
        )}
          ...
          <form onSubmit={criarCliente} className="space-y-6">
            ...
            <div className="bg-purple-900/20 p-4 rounded-lg border border-purple-900 mb-6">
              <label className="block text-sm font-bold text-purple-400 mb-2 flex items-center gap-2">
                <Key size={16} /> Senha Mestra (Sua Segurança)
              </label>
              <input 
                type="password" 
                placeholder="Digite a chave do .env"
                value={masterKey} onChange={e => setMasterKey(e.target.value)}
                className="w-full bg-black border border-purple-800 p-3 rounded text-white focus:outline-none focus:border-purple-500"
                required
              />
            </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input className="w-full bg-gray-800 border border-gray-700 p-3 pl-10 rounded text-white" 
                    placeholder="Ex: Padaria LTDA"
                    value={nomeEmpresa} onChange={e => setNomeEmpresa(e.target.value)} required />
                </div>
              </div>
              <div>
                <label className="text-sm text-gray-400 mb-1 block">CNPJ (Opcional)</label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="text-sm text-gray-400 mb-1 block">Email de Acesso</label>
            ...
            >
              {loading ? 'Criando...' : 'CRIAR ACESSO DO CLIENTE'}
            </button>
