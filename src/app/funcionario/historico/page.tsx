...
export default function MeuHistorico() {
  ...
  const enviarSolicitacao = async () => {
    const motivoOk = Boolean(motivo?.trim());
    const horaOk = Boolean(horaNova);
    const dataOk = modoModal === 'EDICAO' ? true : Boolean(dataNova);

    if (!horaOk || !motivoOk || !dataOk) {
    ...
    try {
      ...
      setFeedback({
        tipo: 'sucesso',
        texto: 'Solicitação enviada! Você pode acompanhar em “Minhas Solicitações”. O administrador irá verificar seu pedido.',
      });
      ...
      setFeedback({ tipo: 'erro', texto: msg || 'Erro ao enviar. Tente novamente.' });
    } finally {
      setEnviando(false);
    }
  };
                ...
                >
                  ...
                  <p
                    className={`text-2xl font-bold tracking-tight ${
                      resumo.saldoPositivo ? 'text-emerald-400' : 'text-rose-400'
                    }`}
                  >
                    {resumo.saldo}
                  </p>
        ...
        {modalAberto && (
          <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-4 sm:p-6">
            ...
            <div className="bg-[#0f172a] border border-slate-700 w-full max-w-sm rounded-3xl shadow-2xl p-6 space-y-5 relative z-10 animate-in slide-in-from-bottom-10 fade-in duration-300">
              ...
              {modoModal === 'EDICAO' && pontoSelecionado && (
                <div className="grid grid-cols-1 gap-3">
                  <div className="space-y-1">
                    <label className="text-[10px] text-slate-400 font-bold uppercase ml-1">Horário batido (original)</label>
                    <input
                      type="text"
                      value={toHHmm(new Date(pontoSelecionado.dataHora))}
                      readOnly
                      className="w-full bg-slate-900/60 border border-white/5 p-4 rounded-2xl text-white text-3xl font-bold text-center outline-none opacity-90 cursor-not-allowed"
                    />
                    <p className="text-[11px] text-slate-500 text-center">Esse horário não pode ser alterado diretamente.</p>
                  </div>
              ...
              {/* Justificativa (para ambos os modos) */}
              <div className="space-y-1">
              ...
              {feedback && (
                ...
                >
                  <div className="flex items-start gap-2">
                    ...
                    <p className="leading-snug">{feedback.texto}</p>
                  </div>
                </div>
              )}

              {/* Botão: após sucesso vira "Ir para Minhas Solicitações" */}
              {feedback?.tipo === 'sucesso' ? (
                <button
                  onClick={irParaSolicitacoes}
                  className="w-full bg-emerald-600 hover:bg-emerald-700 text-white py-4 rounded-xl font-bold text-sm shadow-lg shadow-emerald-900/20 active:scale-95 transition-all flex items-center justify-center gap-2"
                >
                  <CheckCircle2 size={18} /> Ir para Minhas Solicitações
                </button>
              ) : (
                <button
                  onClick={enviarSolicitacao}
                  disabled={enviando}
                  className="w-full bg-purple-600 hover:bg-purple-700 disabled:opacity-60 disabled:cursor-not-allowed text-white py-4 rounded-xl font-bold text-sm shadow-lg shadow-purple-900/20 active:scale-95 transition-all flex items-center justify-center gap-2"
                >
                  {enviando ? (
                    <>
                      <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
