  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-4">
      <div className="max-w-md mx-auto space-y-6">
        ...
        {resumo && (
          <div className="bg-blue-900/20 border border-blue-800 p-4 rounded-xl flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="bg-blue-600 p-2 rounded-lg text-white">
                <Clock size={24} />
              </div>
              <div>
                <p className="text-xs text-blue-300 uppercase font-bold">Total no Período</p>
        ...
        <div className="bg-slate-900 p-4 rounded-xl border border-slate-800 space-y-3">
          <p className="text-xs text-slate-400 font-bold uppercase">Filtrar Período</p>
          <div className="flex gap-2">
            <div className="flex-1">
              <label className="text-[10px] text-slate-500 block mb-1">De:</label>
              <input type="date" value={dataInicio} onChange={(e) => setDataInicio(e.target.value)} className="w-full bg-slate-950 border border-slate-700 p-2 rounded text-white text-sm" />
            </div>
            <div className="flex-1">
              <label className="text-[10px] text-slate-500 block mb-1">Até:</label>
              <input type="date" value={dataFim} onChange={(e) => setDataFim(e.target.value)} className="w-full bg-slate-950 border border-slate-700 p-2 rounded text-white text-sm" />
            </div>
          </div>
          <div className="flex gap-2 pt-2">
            <button onClick={carregar} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-2 rounded-lg font-bold text-sm flex items-center justify-center gap-2"><Search size={16} /> Filtrar</button>
            {/* Passamos o resumoHoras para o PDF aqui 👇 */}
            {pontos.length > 0 && <div className="flex-1"><BotaoRelatorio pontos={pontos} filtro={filtroPDF} resumoHoras={resumo} /></div>}
          </div>
        </div>

        <div className="space-y-3">
          {loading ? <p className="text-center text-slate-500">Carregando...</p> : pontos.length === 0 ? <p className="text-center text-slate-500">Nada encontrado.</p> : (
            pontos.map((ponto) => (
              <div key={ponto.id} className="bg-slate-900 p-4 rounded-xl border border-slate-800 flex justify-between items-center relative overflow-hidden">
                <div className={`absolute left-0 top-0 bottom-0 w-1 ${ponto.tipo === 'ENTRADA' ? 'bg-green-500' : ponto.tipo === 'SAIDA' ? 'bg-red-500' : 'bg-yellow-500'}`} />
                <div className="pl-3">
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500 mt-1 block">{ponto.tipo?.replace('_', ' ') || 'PONTO'}</span>
                </div>
                <div className="text-right">
                  <div className="flex flex-col items-end gap-1">
                    <span className="text-xs text-slate-400 max-w-[150px] truncate block text-right">{ponto.endereco ? ponto.endereco.split(',')[0] : 'GPS'}</span>
                  </div>
                </div>
              </div>
            ))
