...
export default function Home() {
  ...
  const capturarLocalizacao = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) =>
        setStatusMsg({ tipo: "erro", texto: "Erro no GPS: " + error.message })
    );
  };
  ...
  const baterPonto = async () => {
    ...
    if (!imageSrc) {
      ...
      setStatusMsg({
        tipo: "erro",
        texto:
          "🚫 ERRO CRÍTICO: Câmera não detectada! Você precisa permitir o acesso à câmera para bater o ponto.",
      });
    ...
    <main className="flex min-h-screen flex-col items-center justify-center p-4 bg-slate-900 text-white">
      <div className="w-full max-w-md bg-slate-800 rounded-2xl shadow-xl overflow-hidden border border-slate-700">
        ...
        <div className="p-6 space-y-6">
          <div
            className={`relative rounded-xl overflow-hidden border-2 bg-black aspect-video ${
              cameraErro
                ? "border-red-500 shadow-red-900/50 shadow-lg"
                : "border-slate-600"
            }`}
          >
            <Webcam
              ...
              onUserMediaError={() => {
                ...
                setStatusMsg({
                  tipo: "erro",
                  texto: "Câmera bloqueada pelo navegador!",
                });
              }}
            ...
            {cameraErro && (
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-gray-900/90 text-center p-4">
                ...
                <p className="text-xs text-gray-400 mt-2">
                  Verifique as permissões do navegador ou se outra aba está
                  usando a câmera.
                </p>
              </div>
            )}
