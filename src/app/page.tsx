...
import { 
  MapPin, Camera, LogOut, History, Settings, RefreshCcw, 
...
export default function Home() {
  ...
  useEffect(() => {
    if (status === 'unauthenticated') { router.push('/login'); } 
      ...
      if (session?.user?.cargo === 'ADMIN') { router.push('/admin'); }
    }
  ...
  const capturarLocalizacao = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        setStatusMsg({ tipo: 'sucesso', texto: 'GPS Localizado!' });
      },
  ...
  const baterPonto = async () => {
    if (!location) { setStatusMsg({ tipo: 'erro', texto: 'Ligue o GPS primeiro!' }); return; }
    ...
    try {
      const response = await axios.post('/api/bater-ponto', {
        ...
        latitude: location.lat, longitude: location.lng, fotoBase64: imageSrc, tipo: tipoPonto 
      });
    ...
    <main className="min-h-screen bg-slate-950 text-white flex flex-col items-center p-4">
      ...
      <div className="w-full max-w-md space-y-4">
        ...
        <div className="flex justify-between items-center bg-slate-900 p-4 rounded-2xl border border-slate-800 shadow-md">
            ...
            <button onClick={() => signOut()} className="p-3 bg-red-500/10 rounded-xl text-red-400 border border-red-500/20 hover:bg-red-500 hover:text-white transition-all">
                <LogOut size={20} />
            </button>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2">
            ...
            <Link href="/funcionario/assinatura" className="flex flex-col items-center justify-center gap-2 bg-slate-800 hover:bg-slate-700 p-3 rounded-xl border border-slate-700 transition-all active:scale-95 group">
                <div className="bg-purple-900/30 text-purple-400 p-2 rounded-full group-hover:bg-purple-600 group-hover:text-white transition-colors">
                    <PenTool size={20} />
                </div>
                <span className="text-[10px] font-bold uppercase text-slate-300">Assinar</span>
            </Link>

            <Link href="/funcionario/ausencias" className="flex flex-col items-center justify-center gap-2 bg-slate-800 hover:bg-slate-700 p-3 rounded-xl border border-slate-700 transition-all active:scale-95 group">
                <div className="bg-yellow-900/30 text-yellow-500 p-2 rounded-full group-hover:bg-yellow-600 group-hover:text-white transition-colors">
                    <FileText size={20} />
                </div>
            ...
            <Link href="/historico" className="flex flex-col items-center justify-center gap-2 bg-slate-800 hover:bg-slate-700 p-3 rounded-xl border border-slate-700 transition-all active:scale-95 group">
                ...
                <span className="text-[10px] font-bold uppercase text-slate-300">Histórico</span>
            </Link>
        ...
        <div className="bg-slate-800 rounded-2xl shadow-xl overflow-hidden border border-slate-700 p-4 space-y-4">
            ...
            {!location ? (
                ...
                <div className="space-y-3">
                    ...
                    <button onClick={baterPonto} disabled={loading || cameraErro} className={`w-full py-4 rounded-xl font-bold flex items-center justify-center gap-2 text-lg shadow-xl transition-all active:scale-95 ${loading ? 'bg-slate-600' : 'bg-purple-600 hover:bg-purple-700'}`}>
